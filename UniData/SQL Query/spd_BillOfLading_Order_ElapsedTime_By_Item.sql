...
/****** Object:  StoredProcedure [dbo].[spd_BillOfLading_Order_ElapsedTime_By_Item]    Script Date: 2016-12-06 12:02:14 PM ******/
SET ANSI_NULLS ON
GO
...
Day range calculation pre-conditions:
	1. Weekend days and Statutory Holidays will be excluded from the calculation.

------------------------------------------------------------------------------------------------

Business rules:
	1. LoadAndGoFlag -> The order goods are loaded on the truck and will be shipping to the customer on the same day the BOL is cut (this is a new field introduced on year 2010 along with ShippingUserId).
	2. LateOrderReleaseFlag -> A released order is considered to be late if it is released after or on the customer delivery by date.
	3. LateShipmentFlag -> A shipment is considered to be late if:
						   ...
						   c). However, a pick up order is NOT considered to be late if the customer picks up the order the same day as updated delivery by date that is regardless if the shipper changes the LoadAndGoFlag to Y.
	4. DayOffFlag -> Order released after 2:00PM local warehouse time will be considered to be a day off. The actual order release date would be the next working day.
...
Total number of days and range definitions:
    1. UpdatedDelDt - InitialDelDt -> Difference days between the customer delivery by date vs DC updated delivery by date.
	2. DelDt - RelDt -> Total number of working days required from the date the order is released to the customer delivery by date (DeliveryByDate is not counted).
    3. ShpDt - RelDt -> Extra number of working days required from the date the order is released to the date it is shipped out.
    4. DelDt - ShpDt -> Service number of working days required from the date the order is shipped out to the customer delivery by date (DeliveryByDate is not counted and in case the delivery by date happen to be a statutory holiday, the most last working day will be considered to be the actual delivery by date).
	5. ShpDt - OrdDt -> Extra number of working days required from the date the order is issued by customer to the date it is shipped out.
    6. (DelDt - RelDt) Range -> Total number of working days required summarized in range (< 0, 0 to 9, >= 10).
	7. (ShpDt - RelDt) Range -> Extra number of working days required summarized in range (< 0, 0 to 9, >= 10).
    8. (DelDt - ShpDt) Range -> Service number of working days required summarized in range (< 0, 0 to 9, >= 10).
	9. (ShpDt - OrdDt) Range -> Extra number of working days required summarized in range (< 0, 0 to 9, >= 10).
...
ALTER PROCEDURE [dbo].[spd_BillOfLading_Order_ElapsedTime_By_Item]
...
BEGIN
	...
	IF @includeMarketSegment = 0
		SELECT FiscalMonth = BillOfLading.FiscalPeriodCode
			...
			, DayOffFlag =
				CASE 
					WHEN DATEADD(HOUR, WarehouseTimeZone.TimeZoneOffset, SalesOrderPacking.ReleaseDateTime) > DATEADD(HOUR, 14, CONVERT(SMALLDATETIME, DATEDIFF(DAY, 0, DATEADD(DAY, 0, SalesOrderPacking.ReleaseDateTime))))
						THEN 'Yes'
					ELSE
						'No'
				END
			...
			, [DelDt - ShpDt] =
				CASE
					WHEN ISNULL(SalesOrderPacking.DeliveryByDate, CONVERT(DATE, '19000101')) = CONVERT(DATE, '19000101')
					OR ISNULL(BillOfLading.ShippingDate, CONVERT(DATE, '19000101')) = CONVERT(DATE, '19000101')
						THEN NULL
					ELSE 
						(
		    ...
		    , [ShpDt - OrdDt] = 
				CASE
					WHEN ISNULL(BillOfLading.ShippingDate, CONVERT(DATE, '19000101')) = CONVERT(DATE, '19000101')
					OR ISNULL(SalesOrder.CustomerOrderDate, CONVERT(DATE, '19000101')) = CONVERT(DATE, '19000101')
						THEN NULL
					ELSE 
						(
						SELECT 
							(COUNT(1) - 1) *
								CASE WHEN CONVERT(DATE, BillOfLading.ShippingDate) >= CONVERT(DATE, SalesOrder.CustomerOrderDate)
									THEN 1
								ELSE -1
								END
						FROM DataWarehouse.dbo.[Time]
						WHERE DateId BETWEEN
								CASE WHEN SalesOrder.CustomerOrderDate <= BillOfLading.ShippingDate
									THEN CONVERT(DATE, SalesOrder.CustomerOrderDate)
								ELSE 
									CONVERT(DATE, BillOfLading.ShippingDate)
								END 
							AND 
								CASE WHEN SalesOrder.CustomerOrderDate <= BillOfLading.ShippingDate
									THEN CONVERT(DATE, BillOfLading.ShippingDate)
								ELSE 
									CONVERT(DATE, SalesOrder.CustomerOrderDate)
								END 
						AND DATEPART(WEEKDAY, DateId) NOT IN (1, 7)
						AND DateId NOT IN (
												SELECT StatutoryHolidayDate
			...
			, Product.ProductCode
			, Product.ProductName
		    , ShippingWeight = ISNULL(CONVERT(NVARCHAR(15),BillOfLading.ShippingWeight),'')
		FROM DataWarehouse.dbo.BillOfLading
			...
			LEFT JOIN DataWarehouse.dbo.Product ON
				BillOfLading.ComputerizedPartNumber = Product.ComputerizedPartNumber
