...
/****** Object:  StoredProcedure [dbo].[spd_ReserveProductCodes]    Script Date: 2017-03-21 7:47:13 AM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	IF UPPER(@CompanyGroup) = 'ALL' AND @RootForCodes <> '' AND (@TestForRootCodes < '001000' OR @TestForRootCodes > '999999')
	BEGIN
	...
	BEGIN
		...
		AND @NumberOfCodes <= (
				SELECT COUNT(*) FROM 
				[PIM].[dbo].[ProductCodesReservation]
				WHERE @RootForCodes = '' OR [ProductCode] LIKE @RootForCodes + '%'
			)
	END
