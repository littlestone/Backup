...
/****** Object:  StoredProcedure [dbo].[spd_BillOfLading_Order_ElapsedTime]    Script Date: 2017-01-03 2:55:45 PM ******/
SET ANSI_NULLS ON
GO
									...
									WHEN DATEADD(HOUR, WarehouseTimeZone.TimeZoneOffset, SalesOrderPacking.ReleaseDateTime) > DATEADD(HOUR, 14, CONVERT(SMALLDATETIME, DATEDIFF(DAY, 0, DATEADD(DAY, 0, SalesOrderPacking.ReleaseDateTime))))
										THEN 
											CASE WHEN CONVERT(DATE, SalesOrderPacking.DeliveryByDate) >= CONVERT(DATE, SalesOrderPacking.ReleaseDateTime)
												THEN COUNT(1) - 2   -- DAY OFF -> SERVER RELEASETIME + WAREHOUSE TIME ZONE OFFSET > 2:00 PM + ORDER RELEASED ON THE DELIVERY BY DATE WILL BE CONSIDERED A LATE ORDER RELEASE
