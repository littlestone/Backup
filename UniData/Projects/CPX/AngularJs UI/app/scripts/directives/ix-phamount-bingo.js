...
angular.module('cpxUiApp')
  ...
  .controller('ixPhasingAmountCtrl', function ($scope, colorPhasing, resCurrencyRate, resPrjBudget, resPrjForecastEach) {
          ...
          if ($scope.targetCurrencyId === $scope.baseCurrencyId) {
            if ($scope.budgetTypeId === 1) {
              ...
              resPrjForecastEach.query({prjId: $scope.projectId, budYear: $scope.year}, function (data) {
                var forecastData = data[$scope.categoryId];
                // console.log(forecastData);
                originalAmount = forecastData[month];
                $scope.ngModel = originalAmount;
