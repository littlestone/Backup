...
select  prc.name ProductCompanyID,
  ...
  case brand_parent.subtypeid when 1764047 then pimviewapipck.getcontextname(brand_parent.id) else pimviewapipck.getcontextname(brandlnk.classificationid) end Brand,
  case brand_parent.subtypeid when 1764047 then pimviewapipck.getcontextname(brandlnk.classificationid) else null end SubBrand,
  pimviewapipck.getcontextvalue4node(brandlnk.classificationid,1767458) IsOEMBrand,
  pimviewapipck.getname(prdlinelnk.classificationid) ProductLineID
from (((((product_v prc 
  left join productclassificationlink_v brandlnk on brandlnk.productid = prc.id and brandlnk.linktypeid = 1764532)
  ...
  left join productclassificationlink_v prdlinelnk on prdlinelnk.productid = prc.id and prdlinelnk.linktypeid = 1768878)
